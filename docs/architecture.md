...
검사 엔진(@a11ychk/core)의 진입점은 셋이다. **웹 서비스**(아래 파이프라인 — 계정·한도·
보고서 저장), **GitHub Action**(action/ — CI에서 페이지 목록 검사 후 잡 요약·게이트),
**MCP 서버**(packages/mcp — AI 코딩 도구가 localhost 포함 페이지를 직접 검사). 스캔
루프(scanner/scanUrls)·집계(aggregateScan)·모범 사례 권고 분리 판정을 셋이 공유하므로
같은 페이지에는 어느 진입점이든 같은 점수가 나온다.
...
## 수동 판정 축 (WCAG 우선 + KWCAG 파생)

수동검사 가이드·체크리스트(웹 /guide·보고서 수동 섹션·확장)는 **WCAG 2.2 성공기준
축(A/AA)**으로 나열하고, 검사 방법 콘텐츠는 대응 KWCAG 항목(`KWCAG_ITEMS.howToTest`)에서
역매핑(`KWCAG_BY_WCAG`)으로 가져온다. 신규 판정은 `standard='wcag'`로 저장되어
수동·통합 준수율에 직접 반영된다.

- **판정 파생 규칙**(`combineOutcomes`): failed > cannotTell > (대응 항목 전원 판정 시)
...
- KWCAG 고유 항목(5.4.3·6.4.4 — WCAG 대응 없음)은 "KWCAG 추가 항목"으로 유지되며
  `standard='kwcag'`로 저장된다(WCAG 점수에 미반영 — 올바른 동작).
