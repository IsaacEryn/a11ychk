...
| 3 | 크롬 확장 (MV3) — 로그인 필요 페이지 스캔, 수동 점검 체크리스트, 서비스 DB 연동 | ✅ 완료 (apps/extension 0.4.0, 크롬 웹스토어 게시) |
| 4 | 영어/다국어 오픈(en), 정기 스캔 스케줄(Vercel Cron), 접근성 배지 위젯 | ✅ 완료 |
| 5 | 확산·성장 — GitHub Action CI 연동, 비로그인 맛보기 검사, 친구 초대 등급 시스템, 관리자 보안 강화 | ✅ 완료 (아래 상세) |
| 6 | **MCP 서버** (@a11ychk/mcp) — AI 코딩 도구가 검사·한국어 가이드를 직접 호출, localhost 검사 | ✅ 완료 (npm 게시 — @a11ychk/mcp, MCP 레지스트리 등재) |
...
- Phase 6의 MCP 서버와는 방향이 반대다: MCP는 **AI 도구가 우리를 호출**(로컬,
  무료, 검사·가이드 제공)하고, 이 항목은 **우리 서버가 LLM을 호출**(호스팅, 과금,
  수정 코드 생성)한다. MCP가 무료로 커버하는 수요를 확인한 뒤 범위를 다시 잡는다.
