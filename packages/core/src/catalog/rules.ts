...
export const RULE_CATALOG: RuleCatalogEntry[] = [
  ...
  },
  {
    ...
    guide: {
      ko: "`role=\"tab\"` 요소는 어떤 탭인지 이름이 있어야 스크린 리더가 \"공지사항 탭, 1/3\"처럼 읽어 줍니다. 탭 안에 텍스트를 넣거나 `aria-label`로 이름을 지정하세요.\n\n```html\n<div role=\"tablist\">\n  <button role=\"tab\" aria-selected=\"true\" aria-controls=\"panel-notice\">공지사항</button>\n  <button role=\"tab\" aria-selected=\"false\" aria-controls=\"panel-faq\">자주 묻는 질문</button>\n</div>\n```",
      en: "Give every role=\"tab\" element an accessible name via its text content or aria-label.",
    },
  },
  {
